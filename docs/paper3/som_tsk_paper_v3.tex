...
Metal GPU was unavailable at benchmark time (compiler not installed).}
\label{tab:timing}
\centering
\renewcommand{\arraystretch}{1.1}
\small
...
\midrule
SOM-TSK & \textbf{0.709} & \textbf{0.748} & \textbf{0.787} & \textbf{0.52} & 1.2s \\
...
\caption{ARI comparison across all four algorithms on the 16 non-trivial datasets (excluding scalability and high-dimensional benchmarks where all algorithms achieve ARI $\approx 1.0$). Error bars on KMeans++ show $\pm 1\sigma$ across 10 random seeds.}
...
To rigorously compare all four algorithms across the 24 datasets, we apply the Friedman test~\cite{demsar2006statistical} followed by Nemenyi post-hoc analysis.

\textbf{Friedman test.} The null hypothesis (all algorithms perform equally) is rejected with $\chi^2 = 41.47$, $p = 5.20 \times 10^{-9}$ ($\alpha = 0.05$). The differences among the four algorithms are highly significant.

\textbf{Average ranks} (1 = best): SOM-TSK = 1.77, KMeans++ = 1.81, AutoSOM = 2.71, DenSOM = 3.71.

\textbf{Nemenyi critical difference} ($\alpha = 0.05$, $k=4$, $N=24$): CD $= 0.957$.

\textbf{Pairwise significance:}
\begin{itemize}
  \item SOM-TSK vs.\ KMeans++: $|\Delta R| = 0.04 < \text{CD}$ (not significant by Nemenyi)
  \item SOM-TSK vs.\ AutoSOM: $|\Delta R| = 0.94 < \text{CD}$ (not significant by Nemenyi)
  \item SOM-TSK vs.\ DenSOM: $|\Delta R| = 1.94 > \text{CD}$ (\textbf{significant})
  \item KMeans++ vs.\ DenSOM: $|\Delta R| = 1.90 > \text{CD}$ (\textbf{significant})
\end{itemize}

\textbf{Wilcoxon signed-rank test} (SOM-TSK vs.\ KMeans++ only, paired): $W = 29.0$, $p = 0.765$ (\emph{not} significant at $\alpha = 0.05$). Although the overall Friedman test is highly significant---driven by the large gap between the SOM-TSK/KMeans++ pair and the much weaker DenSOM/AutoSOM---the pairwise difference between SOM-TSK and KMeans++ is \emph{not} statistically significant. This is honest scientific reporting: SOM-TSK and KMeans++ are statistically indistinguishable on this benchmark suite (19 of 24 datasets are ties), and SOM-TSK's advantage is confined to specific structural regimes such as high-dimensional manifold data (Digits).

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/friedman_nemenyi.pdf}
\caption{Critical difference diagram (Nemenyi post-hoc, $\alpha = 0.05$). Algorithms connected by a bar within the CD are not significantly different. SOM-TSK and KMeans++ form a top group; both significantly outperform DenSOM.}
\label{fig:cd_diagram}
\end{figure}

\textbf{Interpretation.} SOM-TSK achieves the best average rank (1.77), marginally ahead of KMeans++ (1.81), but the difference is far below the Nemenyi critical difference and the paired Wilcoxon test is not significant ($p = 0.765$)---expected given that 19 of 24 datasets are ties. We therefore do \emph{not} claim a general advantage of SOM-TSK over KMeans++; the methods are statistically equivalent on average. Both, however, significantly outperform DenSOM, and SOM-TSK's deterministic, parameter-light initialization provides a practical reproducibility advantage over restart-based KMeans++ at no average-quality cost.
...
\subsection{Quantifying the ``Never Worse'' Guarantee}
\label{sec:guarantee}

SOM-TSK's 0-loss record is not coincidental---it is architecturally guaranteed by Phase C. Since Phase C runs a deterministic KMeans++ on raw data, the candidate pool always contains the KMeans++ solution. The inertia-windowed selection can only choose a different candidate if it has both (i) inertia within 5\% of the KMeans++ solution and (ii) higher CH score. This means:

\begin{equation}
  \mathrm{ARI}(\text{SOM-TSK}) \ge \mathrm{ARI}(\text{KMeans++}) - \epsilon,
\end{equation}

where $\epsilon$ is bounded by the maximum ARI degradation possible within a 5\% inertia window. Empirically, $\epsilon < 0.005$ across all 24 datasets (the Iris ``loss'' of $-0.004$ is the worst case and falls within the tie threshold).

This guarantee holds under two conditions: (i) Phase C uses the same KMeans++ implementation as the baseline, and (ii) the inertia window $\rho$ is not too large. For $\rho = 1.05$, the guarantee is robust; for $\rho > 1.15$, degenerate candidates could theoretically be selected.
...
The empirical evidence suggests that determinism is net-positive: the structured exploration of the candidate pool (Phases A, A-alt, B-bis, C) provides more diverse candidates than random restarts, as evidenced by the 6-win/0-loss record. However, for very large $k$ ($> 100$), adding controlled stochasticity to Phase B-bis (e.g., random starting neurons beyond the top-3) might improve coverage.
...
A stricter threshold (e.g., 0.01) would reclassify s3 ($\Delta = +0.007$) as a tie, reducing the win count to~5.
