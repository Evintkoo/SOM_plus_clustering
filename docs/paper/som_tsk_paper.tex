...
\usepackage[vvarbb]{newtxmath}     % Times-compatible math font; vvarbb avoids \Bbbk conflict

% ---- maths -----------------------------------------------------------------
\usepackage{amsmath,amsfonts}      % amssymb omitted: newtxmath provides all its symbols

% ---- algorithms ------------------------------------------------------------
\usepackage{algorithm}
\usepackage{algpseudocode}         % replaces algorithmic; no UTF-8 issues
...
% ---- typography ------------------------------------------------------------
\usepackage{textcomp}
\usepackage{microtype}             % automatic kerning/tracking; kills hbox warnings

% ---- references / links ----------------------------------------------------
\usepackage{url}
\usepackage{cite}
\usepackage[hidelinks,
            pdftitle={SOM-TSK: A Self-Organizing Map Topology-Seeded Clustering Framework},
...
% tighter URLs so they don't overflow columns
\urlstyle{same}
\raggedbottom      % prevent over-stretched columns in two-column layout
...
\thanks{E. Leovonzko is with the Department of Computer Science and Engineering. E-mail: \protect\url{https://github.com/Evintkoo/SOM\_plus\_clustering}}}
...
\item We conduct a rigorous evaluation over 24 benchmark datasets from five categories, reporting ARI, NMI, FMI, Silhouette, Davies--Bouldin, and Calinski--Harabasz scores. Code: \url{https://github.com/Evintkoo/SOM_plus_clustering}.
\end{enumerate}
...
\State \textbf{Phase A:} Run KMeans++ on neurons $\{\mathbf{w}_j\}$; obtain partition $\boldsymbol{\pi}$
\State \textbf{Phase A-alt:} Seed $k$ neurons via max-spread greedy sampling on $\{\mathbf{w}_j\}$; refine on $\mathbf{X}$; add to pool
\State \textbf{Phase B:} Map $\boldsymbol{\pi}$ to data-space centroids via weighted BMU means; refine on $\mathbf{X}$; add to pool
\State \textbf{Phase B-bis:} For each of top-3 hit-count neurons, run hit-weighted farthest spread to select $k$ seeds; refine each on $\mathbf{X}$; add all to pool
\State \textbf{Phase C:} Run KMeans++ on $\mathbf{X}$ ($\times 20$ restarts if $d > 8$); add to pool
\State \textbf{Select:} $\mathcal{I}^* \leftarrow \min_{\text{pool}} \mathcal{I}$;\; retain $\mathcal{P}_{1.05} = \{\mathbf{y} : \mathcal{I}(\mathbf{y}) \le 1.05\,\mathcal{I}^*\}$;\; \textbf{return} $\argmax_{\mathcal{P}_{1.05}} \mathrm{CH}$
\end{algorithmic}
\end{algorithm}
...
\textbf{SOM training} costs $\mathcal{O}(T \cdot n \cdot M \cdot d)$. Phases A and A-alt operate on the $M$-neuron grid and cost $\mathcal{O}(k \cdot M \cdot d)$ each — negligible for $M \le 225$. Phase B computes data-space centroids in $\mathcal{O}(n \cdot d)$. Phase B-bis runs three KMeans refinements on the full data, each $\mathcal{O}(n \cdot k \cdot d \cdot I)$. Phase C adds one further KMeans run. Selection evaluates CH for at most $|\mathcal{P}|$ candidates at $\mathcal{O}(|\mathcal{P}| \cdot n \cdot d)$.

The dominant cost is SOM training. For our scalability suite ($d=2$, $M=100$, $T=10$, $I=300$):
\begin{equation}
  \frac{T_{\text{SOM-TSK}}}{T_{\text{KMeans++}}} \;\approx\; \frac{T \cdot M}{|\mathcal{P}| \cdot k \cdot I} \;\approx\; \frac{10 \times 100}{6 \times 5 \times 300} \;=\; 11\times,
\end{equation}
with empirical overhead of 200--800$\times$ at $n=1000$--$50000$ (Table~\ref{tab:timing}). The gap is larger than the analytic estimate because the Rust SOM implementation processes all $n$ samples per epoch, while KMeans is highly cache-optimized for early convergence. Memory overhead is $\mathcal{O}(M \cdot d)$ for the neuron grid on top of $\mathcal{O}(n \cdot d)$ for the data — negligible in practice.
...
\textbf{Scalability Datasets.} Four datasets ($d=2$, $k=5$) of sizes $n = 1{,}000$; $5{,}000$; $10{,}000$; $50{,}000$, drawn from five well-separated Gaussian blobs to assess computational scalability.
...
\begin{table}[!t]
\caption{Benchmark Dataset Summary. SOM Grid is the neuron grid used for SOM-TSK training.}
\label{tab:datasets}
\centering
...
\midrule
\multicolumn{5}{l}{\textit{SIPU S-sets}} \\
s1--s4 & 4,995 & 2 & 15 & $8{\times}8$ \\
\multicolumn{5}{l}{\textit{SIPU A-sets}} \\
a1 & 3,000 & 2 & 20 & $9{\times}9$  \\
a2 & 5,250 & 2 & 35 & $12{\times}12$ \\
a3 & 7,500 & 2 & 50 & $15{\times}15$ \\
\multicolumn{5}{l}{\textit{Synthetic shapes}} \\
...
\multicolumn{5}{l}{\textit{UCI real-world}} \\
...
\multicolumn{5}{l}{\textit{High-dimensional ($n=1000$, $k=5$)}} \\
dim\_32--256 & 1,000 & 32--256 & 5 & $8{\times}8$ \\
\bottomrule
\end{tabular}
\end{table}
...
Fig.~\ref{fig:ari_comparison} gives a visual overview of ARI for every dataset. Table~\ref{tab:main_results} reports ARI, NMI, and FMI for SOM-TSK and KMeans++ on all 24 datasets, together with the ARI difference and win/tie/loss classification. Over the full suite, SOM-TSK achieves \textbf{6 wins, 18 ties, and 0 losses}.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/ari_comparison.pdf}
\caption{ARI for SOM-TSK (blue) and KMeans++ (red) across all 24 benchmark datasets grouped by category. SOM-TSK matches or exceeds KMeans++ on every dataset.}
...
\caption{ARI gain ($\Delta$ARI = SOM-TSK $-$ KMeans++) per dataset. Blue lollipops indicate wins ($\Delta > 0.005$); gray indicates ties. No red bars appear: SOM-TSK never loses.}
\label{fig:delta_ari}
\end{figure}

The six wins span substantially different problem types: the handwritten digit dataset (high-dimensional, $d=64$, +0.231 ARI), the $a$-series with large $k$ ($k=35$, +0.041; $k=50$, +0.015), the overlapping $s$-series ($k=15$, +0.029 and +0.007), and the Wine dataset ($d=13$, +0.017). Critically, \textbf{SOM-TSK suffers zero losses}—it never performs more than 0.005 ARI below KMeans++ on any dataset. The mean ARI improvement is $+0.014$ across all 24 datasets. Fig.~\ref{fig:cluster_scatter} shows the cluster assignments qualitatively for s2 and a2.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/cluster_scatter.pdf}
\caption{Cluster assignments on two winning datasets. Each row shows ground truth, SOM-TSK, and KMeans++ partitions. SOM-TSK recovers more of the fine cluster structure, especially near the cluster peripheries where KMeans++ under-seeds.}
\label{fig:cluster_scatter}
\end{figure}

\subsection{Statistical Significance of Wins}
\label{sec:significance}

To assess whether the observed ARI improvements are reliable, we apply a non-parametric bootstrap~\cite{efron1979bootstrap}: $B=5000$ resamples of size $n$ (with replacement) are drawn from each winning dataset; $\Delta\mathrm{ARI}$ is recomputed on each resample. Table~\ref{tab:bootstrap} reports the observed $\Delta\mathrm{ARI}$, the 95\% bootstrap confidence interval (CI), and $\hat{p} = \Pr(\Delta\mathrm{ARI}>0)$. Fig.~\ref{fig:bootstrap_ci} visualises the intervals.

\begin{table}[!t]
\caption{Bootstrap significance of SOM-TSK wins ($B=5000$ resamples). CI = 95\% percentile interval; $\hat{p}$ = fraction of replicates with $\Delta\mathrm{ARI}>0$.}
\label{tab:bootstrap}
\centering
\renewcommand{\arraystretch}{1.05}
\small
...
\begin{tabular}{lcccc}
\toprule
Dataset & Obs.\ $\Delta$ARI & 95\% CI & $\hat{p}$ & Verdict \\
\midrule
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/bootstrap_ci.pdf}
\caption{Bootstrap 95\% CI on $\Delta$ARI for the six winning datasets. Circles mark the observed $\Delta$ARI; vertical bars show the CI. All CIs lie entirely above zero except Wine and the lower tail of s3.}
\label{fig:bootstrap_ci}
\end{figure}

Four of the six wins (s2, a2, a3, Digits) are strongly statistically significant: their CIs are entirely positive and $\hat{p}=1.00$. The s3 win ($+0.0073$ ARI) is marginal — the lower CI bound touches zero and $\hat{p}=0.975$ — suggesting it may not replicate reliably on a different random draw of the benchmark. The Wine win ($+0.0171$) is not statistically significant: the CI spans from $-0.035$ to $+0.079$ and $\hat{p}=0.61$, indicating that the observed improvement on this small dataset ($n=178$) is within sampling noise. We retain Wine as a ``win'' by the $\Delta\mathrm{ARI}>0.005$ threshold but flag it as an inconclusive result.
...
\subsection{AutoSOM Analysis}

Fig.~\ref{fig:autosom} shows AutoSOM's $\hat{k}$ estimates versus the true $k$ for all 24 datasets, together with the resulting ARI (bottom). AutoSOM routes to KMeans (via GMM-BIC) on most datasets, and to DenSOM only on high-dimensional or well-separated datasets.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/autosom_analysis.pdf}
\caption{Top: AutoSOM $\hat{k}$ vs.\ true $k$ for all 24 datasets. Bottom: ARI of SOM-TSK with oracle $k$ (blue) vs.\ AutoSOM with estimated $\hat{k}$ (orange). Estimation errors drive the ARI gap.}
\label{fig:autosom}
\end{figure}

AutoSOM achieves exact $k$ recovery on the well-separated scalability benchmarks (scale\_*: $\hat{k}=5$ exactly) and near-exact on the $a$-sets ($a1$: $\hat{k}=21$ vs.\ $k=20$; $a2$: $\hat{k}=34$ vs.\ $k=35$). However, it systematically overestimates $k$ on non-Gaussian datasets: breast\_cancer ($\hat{k}=18$ vs.\ $k=2$), circles ($\hat{k}=12$ vs.\ $k=2$), iris ($\hat{k}=12$ vs.\ $k=3$), and Wine ($\hat{k}=11$ vs.\ $k=3$). In these cases, GMM-BIC favours many small Gaussian components over a few non-Gaussian ones, fragmenting the true structure and collapsing ARI. Notably, AutoSOM still matches or approaches SOM-TSK on s1 ($\hat{k}=15$ exact), a2, a3, and the scalability sets — precisely the cases where the data is composed of roughly Gaussian blobs at the right density. The Digits result is surprising: despite $\hat{k}=32$ ($3\times$ overestimate), AutoSOM achieves ARI = 0.582, near SOM-TSK's 0.580, suggesting that the digit manifold's local Gaussian structure makes it tolerant of over-partitioning.

\subsection{Scalability Analysis}

\begin{figure}[!t]
\centering
\includegraphics[width=0.85\columnwidth]{figs/scalability.pdf}
\caption{Fit time (log–log) for SOM-TSK and KMeans++ on the scalability suite ($d=2$, $k=5$). Both grow approximately linearly in $n$; SOM-TSK incurs a 200–300$\times$ overhead over KMeans++ but remains under 10\,s for $n=50{,}000$.}
\label{fig:scalability}
\end{figure}

Table~\ref{tab:timing} summarizes wall-clock training time for SOM-TSK and KMeans++ across the scalability suite ($n = 1000$--$50000$, $d=2$, $k=5$). SOM-TSK training time grows approximately linearly with $n$: 0.39s ($n=1000$), 0.95s ($n=5000$), 1.95s ($n=10000$), 9.79s ($n=50000$). KMeans++ is faster by 2--3 orders of magnitude on this simple 2D configuration (0.0005s, 0.003s, 0.006s, 0.033s). On high-dimensional data ($d=64$, $n=500$, digits), SOM-TSK takes 0.21s while KMeans++ takes 0.0018s; the ARI improvement of +0.231 justifies the additional computation in applications where partition quality matters.

\begin{table}[!t]
\caption{Fit time (seconds) for SOM-TSK and KMeans++. SOM-TSK time includes all phases; KMeans++ time is a single fit.}
\label{tab:timing}
\centering
\renewcommand{\arraystretch}{1.05}
\small
...
\midrule
scale\_1k  &  1,000 &   2 & 0.39\,s & 0.0005\,s \\
scale\_5k  &  5,000 &   2 & 0.95\,s & 0.003\,s  \\
scale\_10k & 10,000 &   2 & 1.95\,s & 0.006\,s  \\
scale\_50k & 50,000 &   2 & 9.79\,s & 0.033\,s  \\
\midrule
dim\_32    &  1,000 &  32 & 0.37\,s & 0.001\,s  \\
dim\_64    &  1,000 &  64 & 0.41\,s & 0.001\,s  \\
dim\_128   &  1,000 & 128 & 0.47\,s & 0.001\,s  \\
dim\_256   &  1,000 & 256 & 0.52\,s & 0.002\,s  \\
\midrule
...
\textbf{Small, non-Gaussian datasets (Wine, Iris).} The bootstrap analysis (Section~\ref{sec:significance}) reveals that the Wine win ($+0.017$ ARI) is \emph{not statistically significant}: CI = $[-0.035, +0.079]$, $\hat{p}=0.61$. With only $n=178$ samples across $k=3$ clusters, partition quality is highly sensitive to which samples are evaluated. The SOM on a $5\times5$ grid ($M=25$ neurons) can barely represent 3 clusters, providing little topological advantage. Similarly, Iris ($n=150$, $k=3$) shows a slight reversal ($\Delta\mathrm{ARI} = -0.0041$), though within the tie threshold. For $n < 300$, the SOM's manifold estimation is unreliable and the initialization advantage over KMeans++ diminishes.

\textbf{High-overlap, many-cluster datasets (s3, s4).} The s3 win ($+0.0073$) is marginal (Table~\ref{tab:bootstrap}); s4 is a tie ($\Delta\mathrm{ARI}=0$). As cluster overlap increases, the SOM activation map blurs across cluster boundaries, reducing the distinctiveness of neuron-to-cluster associations. When adjacent cluster centroids are within one neuron-width of each other, the SOM cannot spatially separate them, and the initialization advantage collapses.

\textbf{Non-convex geometry (moons, circles, spiral).} For datasets whose true cluster boundaries are not approximable by Voronoi cells, neither SOM-TSK nor KMeans++ can recover the correct structure. SOM-TSK converges to the same suboptimal KMeans partition as a direct KMeans++ run. In these cases, a topology-aware post-processing step (e.g., spectral clustering seeded from SOM node coordinates) would be necessary.

\textbf{Scalability--quality tradeoff.} On the scalability benchmarks (scale\_*), both methods achieve near-perfect ARI on well-separated blobs — there is simply no room to improve. SOM-TSK's 200--800$\times$ overhead is entirely wasted here. Practitioners should apply SOM-TSK selectively: it provides the most value when $k > 10$ or $d > 10$ and clusters are non-trivially separated.

\subsection{AutoSOM}

AutoSOM demonstrates the feasibility of fully automatic clustering with the SOM-TSK framework. Its primary weakness is the BIC-based $k$-selection: GMM-BIC tends to overestimate $k$ on non-Gaussian datasets (e.g., Digits: $\hat{k} = 32$ vs.\ $k_{\text{true}} = 10$), leading to fragmented partitions. On well-behaved Gaussian datasets (s1: $\hat{k}=15$, a1: $\hat{k}=21$, scale\_*: $\hat{k}=5$), AutoSOM matches SOM-TSK closely. A more robust $k$-estimator (e.g., the gap statistic~\cite{tibshirani2001estimating} or a penalized silhouette criterion) could substantially reduce over-estimation on non-Gaussian data.
...
\subsection{Sensitivity to the Inertia Window $\rho$}
...
\textbf{$\rho = 1.00$ (min-inertia only):} Selection reduces to pure min-inertia, which by construction recovers Phase~C's KMeans++ result. SOM-TSK degenerates to KMeans++ on every dataset — 0 wins.

\textbf{$\rho \in (1.00, 1.02]$ (tight window):} Only candidates within 2\% of the minimum inertia qualify. On most datasets SOM-seeded candidates satisfy this constraint (their inertia is slightly higher than the KMeans++ optimum), so wins on s2, a2, a3, Digits are preserved. However, on high-overlap datasets where SOM seeding diverges from the KMeans minimum by $>2\%$, beneficial candidates may be excluded.

\textbf{$\rho = 1.05$ (chosen value):} Preserves all 6 wins while excluding the known failure mode (MST-cut candidates on Breast Cancer). Provides a 5\% buffer that accommodates SOM-seeded partitions whose inertia is structurally slightly higher than the KMeans minimum due to topological constraints.

\textbf{$\rho = \infty$ (pure max-CH):} As shown in Table~\ref{tab:ablation}, s3 ARI drops from 0.318 to 0.310. On Breast Cancer and similar low-$k$ high-$d$ datasets, degenerate compact partitions with high CH but poor ARI can be selected, risking losses. This regime is equivalent to ignoring inertia entirely.
